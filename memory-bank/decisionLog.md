...
### ADR-006: Suspense と ErrorBoundary の採用
...
- データフェッチのパターンの標準化が必要
- ローディングとエラー状態の一貫した処理が必要
- コンポーネントの責務の明確化が必要

**決定**:
TanStack Query と Suspense/ErrorBoundary を組み合わせたデータフェッチパターンを採用

**理由**:

- React の公式推奨パターンとの整合性
- コードの簡潔化
- エラーハンドリングの統一
- 宣言的なローディング状態の管理
...
- より簡潔なコンポーネント実装
- 一貫したローディング/エラー UI
- Presentational/Container パターンの廃止
- データフェッチの責務の明確化

### ADR-007: TanStack Query の採用
...
- 効率的なデータフェッチングと状態管理が必要
- キャッシュ戦略の標準化が必要
- サーバー状態と UI 状態の明確な分離が必要

**決定**:
TanStack Query をデータフェッチングライブラリとして採用
